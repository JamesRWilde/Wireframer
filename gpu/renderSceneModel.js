import { fillNormalized3 } from "./fillNormalized3.js";
import { fillColor01 } from "./fillColor01.js";
...
export function renderSceneModel(gl, model, params, shaderPack, bufferStore, tmpArrays) {
  ...
  const buffers = bufferStore.getModelBuffers(model);
  if (!buffers) return false;

  if (params.dynamic === true && !bufferStore.updateDynamicBuffers(model, buffers)) return false;
  ...
  if (fillAlpha > 0.001) {
    ...
    setProjectionUniforms(gl, fillLoc, params);
  ...
  if (wireAlpha > 0.001) {
    ...
    setProjectionUniforms(gl, wireLoc, params);
