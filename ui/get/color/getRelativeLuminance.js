...
import { getLinearChannel }from '@ui/get/color/getLinearChannel.js';
...
export function getRelativeLuminance(rgb) {
  ...
  const r = getLinearChannel(rgb[0]);
  const g = getLinearChannel(rgb[1]);
  const b = getLinearChannel(rgb[2]);
