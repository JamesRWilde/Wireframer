...
import { utilLinearChannel }from '@ui/get/color/utilLinearChannel.js';
 ...
 */
export function utilRelativeLuminance(rgb) {
  // Convert sRGB channels to linear RGB
  const r = utilLinearChannel(rgb[0]);
  const g = utilLinearChannel(rgb[1]);
  const b = utilLinearChannel(rgb[2]);
