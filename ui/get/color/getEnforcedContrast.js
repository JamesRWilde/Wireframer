...
import { getContrastRatio }from '@ui/get/color/getContrastRatio.js';
 ...
 */
export function getEnforcedContrast(fg, bg, minRatio) {
  ...
  for (let i = 1; i <= 24; i++) {
    ...
    const towardWhite = getMixedRgb(fg, [255, 255, 255], t);
    const cw = getContrastRatio(towardWhite, bg);
    if (cw > bestRatio) {
      bestRatio = cw;
      best = towardWhite;
    }
    ...
    const towardBlack = getMixedRgb(fg, [0, 0, 0], t);
    const cb = getContrastRatio(towardBlack, bg);
    if (cb > bestRatio) {
      bestRatio = cb;
      best = towardBlack;
    }
