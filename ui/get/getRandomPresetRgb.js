...
import { utilHsvToRgb }from '@ui/get/color/utilHsvToRgb.js';
import { utilRelativeLuminance }from '@ui/get/color/utilRelativeLuminance.js';
...
export function getRandomPresetRgb() {
  for (let i = 0; i < 18; i++) {
    ...
    const rgb = utilHsvToRgb(hue, saturation, value);

    const max = Math.max(rgb[0], rgb[1], rgb[2]);
    const min = Math.min(rgb[0], rgb[1], rgb[2]);
    const spread = max - min;
    const lum = utilRelativeLuminance(rgb);
  ...
  return utilHsvToRgb(Math.random(), 0.82, 0.86);
}
