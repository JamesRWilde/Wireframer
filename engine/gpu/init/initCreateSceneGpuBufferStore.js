 ...
 * - updateDynamicBuffers(model, buffers): Updates the dynamic fill position and normal buffers for the given model and buffer set.
 *
 ...
 */
import { getModelBuffers } from '../get/getModelBuffers.js';
import { updateDynamicBuffersStore } from '../set/updateDynamicBuffersStore.js';

export function initCreateSceneGpuBufferStore(gl, supportsUint32) {
  ...
  return {
    getModelBuffers: (model) => getModelBuffers(gl, modelCache, supportsUint32, model),
    updateDynamicBuffers: (model, buffers) => updateDynamicBuffersStore(gl, model, buffers)
  };
}
