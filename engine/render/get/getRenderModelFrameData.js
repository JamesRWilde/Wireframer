...
import { getRenderWorkerTransformSync } from '../get/getRenderWorkerTransformSync.js';

export function getRenderModelFrameData(model) {
  ...
  setRenderWorkerSend(flatV, flatR, fov, halfW, halfH, modelCy, state.RENDER_FRAME_ID);

  const cached = getRenderCachedTransformResult();
  if (cached?.T && cached?.P2) {
    const converted = getRenderConvertFlatToNested(cached.T, cached.P2, vertexCount);
    T = converted.T;
    P2 = converted.P2;
  } else {
    const result = getRenderWorkerTransformSync(V, Rmat, fov, halfW, halfH, modelCy, vertexCount);
    T = result.T;
    P2 = result.P2;
  }
