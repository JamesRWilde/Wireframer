 ...
 */
export function utilTriCornerNormals(model, triFaces) {
  ...
  for (const tri of triFaces) {
    ...
    for (let c = 0; c < 3; c++) {
      ...
      const [nx, ny, nz] = utilSummedNormals(nRef, faceNormals, adjacent, cosThreshold, smoothGroups, refGroup);
